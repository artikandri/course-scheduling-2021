...
datasetUnitime = pd.read_excel(
    datasetUnitimeFilePath, sheet_name='Sheet1', usecols="AO, AP, AQ")
datasetUnitime.dropna(inplace=True)

# read instructor file
instructorFile = pd.read_csv(instructorFilePath, usecols=[
                             'instructors'], sep=";")
...
datasetIllinois = pd.read_csv(datasetIllinoisFilePath, usecols=[
                              'Name', "CRN"], sep=",").drop_duplicates()
coursesNames = datasetIllinois['Name'].to_list()
coursesDepts = datasetIllinois['CRN'].to_list()
...
uniqueCoursesDepts = list()
filteredCourses = list()
for index, course in enumerate(coursesNames):
    if course not in filteredCourses: 
        filteredCourses.append(course)
        uniqueCoursesDepts.append([course.replace("&amp;", "&"), coursesDepts[index]])

# separate the zipped courses & dept, get the first two digit of CRN as department names
courses, depts=zip(*uniqueCoursesDepts[:len(weeklyHours)])
courses, depts=(list(courses),list(depts))
depts = list(map(lambda dept: str(dept)[:2], coursesDepts[:len(weeklyHours)]))
...
while i != len(courses):
    writer.writerow([courses[i:i+1], weeklyHours[i:i+1],
                    courseInstructors[i:i+1], depts[i:i+1]])
    i += 1
targetFile.close()
